...
fixtures = {
    ...
    createTokensForUser: function createTokensForUser() {
        return db.knex('clients').insert(DataGenerator.forKnex.clients).then(function () {
            return db.knex('accesstokens').insert(DataGenerator.forKnex.createToken({id:3, user_id: 3}));
        }).then(function () {
            return db.knex('refreshtokens').insert(DataGenerator.forKnex.createToken({id:4, user_id: 3}));
        });
    },

    createMoreTokens: function createMoreTokens() {
        return db.knex('accesstokens').insert([
                DataGenerator.forKnex.createToken({id: 1, user_id: 1}),
                DataGenerator.forKnex.createToken({id: 2, user_id: 1})
            ]
        );
    },
...
module.exports = {
    ...
    context: {
        ...
        owner:      {context: {user: 1, token: 1}},
        admin:      {context: {user: 2, token: 99}},
        editor:     {context: {user: 3, token: 3}},
